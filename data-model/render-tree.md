...
* When a render object is attached to the `PipelineOwner` \(via `RenderObject.attach`\), it adds itself to all relevant dirty lists \(e.g., `RenderObject.markNeedsLayout`, `RenderObject.markNeedsPaint`\).
  * All render objects are clean when they're first attached. The initial painting, layout, and semantics passes are driven by a different flow \(via `RenderObject.scheduleInitialLayout`, `RenderObject.scheduleInitialPaint`, and `RenderObject.scheduleInitialSemantics`\). If a render object is dirty at this point, it's likely the result of being detached, mutated, and then reattached.
    * Does this only apply to the VERY first time they're attached? What about when they're attached later on in the apps lifecycle? \(seems like they're painting themselves in this case, and only the very first initial attachment is special\) Also need to add a note as to why cleaning the render view cleans the rest of the tree \(layout / painting flows down\).
  * The initial rendering pass takes place when the `RenderView` is first constructed \(via `RendererBinding.initRenderView`\). This method invokes `RenderView.prepareInitialFrame`, which schedules initial layout and painting \(semantics follows a slightly different flow\).
